...
void Renderer2D::drawTriangle(float xPos, float yPos, float sideLength, float depth, float rotationAngle) {
	if (shouldFlush(3, 3))
		flushBatch();
	...
	float height = (sideLength * sqrt(3.0f)) / 2.0f; // Height of an equilateral triangle

	// Calculate rotation components
	float sinRot = sin(rotationAngle);
	float cosRot = cos(rotationAngle);

	// Vertex 0 (top, rotated)
	m_vertices[m_currentVertex].pos[0] = xPos + height * sinRot;
	m_vertices[m_currentVertex].pos[1] = yPos + height * cosRot;
	...
	// Vertex 1 (bottom left, rotated)
	m_vertices[m_currentVertex].pos[0] = xPos - halfLength * cosRot - halfLength * sinRot;
	m_vertices[m_currentVertex].pos[1] = yPos + halfLength * sinRot - halfLength * cosRot;
	m_vertices[m_currentVertex].pos[2] = depth;
	m_vertices[m_currentVertex].pos[3] = (float)textureID;
	m_vertices[m_currentVertex].color[0] = m_r;
	m_vertices[m_currentVertex].color[1] = m_g;

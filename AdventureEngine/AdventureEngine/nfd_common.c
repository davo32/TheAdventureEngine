...
int32_t NFDi_UTF8_Strlen( const nfdchar_t *str )
{
    ...
    if (strlen(str) > 2) {
        // Use strncpy_s to safely copy the potential BOM
        errno_t err = strncpy_s(maybe_bom, sizeof(maybe_bom), str, 3);
        if (err != 0) {
            // Handle error (e.g., log or throw an exception)
            return;
        }
        maybe_bom[3] = '\0'; // Ensure null termination

        if (strcmp(maybe_bom, NFD_UTF8_BOM) == 0) {
            i += 3;
        }
    }

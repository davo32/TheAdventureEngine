...
static int AppendExtensionToSpecBuf( const char *ext, char *specBuf, size_t specBufLen )
{
    ...
    if ( strlen(specBuf) > 0 )
    {
        // Use strncat_s to safely append the separator
        errno_t err = strncat_s(specBuf, specBufLen, SEP, _TRUNCATE);
        if (err != 0) {
    ...
    // Use strncat_s to safely append the extension wildcard
    errno_t err = strncat_s(specBuf, specBufLen, extWildcard, _TRUNCATE);
    if (err != 0) {
        return -1; // Handle error
    }
